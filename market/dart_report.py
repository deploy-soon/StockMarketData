...
class Report:
    ...
    def get_document(self, rcpNo, dcmNo):
    ...
    def get_report(self, href):
        ...
        if r.status_code != 200:
            self.logger.warning("GET error: {}".format(r.url))
            return None
        rcpNo, dcmNo = self._get_report_document(str(r.content))
        if rcpNo is None or dcmNo is None:
            return None

        return self.get_document(rcpNo, dcmNo)
